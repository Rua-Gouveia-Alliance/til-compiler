...
protected:
  ...
  void processPredicateBinaryExpression(cdk::binary_operation_node *const node, int lvl,
                                        bool doubles, bool pointer);
  void processArithmeticBinaryExpression(cdk::binary_operation_node *const node, int lvl,
                                         bool doubles, bool pointer);
  template <typename T> void process_literal(cdk::literal_node<T> *const node, int lvl) {}
  bool equal_types(std::shared_ptr<cdk::basic_type> t1, std::shared_ptr<cdk::basic_type> t2,
                   bool strict = false);
