...
void til::xml_writer::do_print_node(til::print_node * const node, int lvl) {
  ...
  os() << " newline='" << (node->ln() ? "true" : "false") << "'>" << std::endl;
...
void til::xml_writer::do_function_def_node(til::function_def_node * const node, int lvl) {
  // ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') + "<" + node->label();
  os() << " type='" << type_to_str(node->type()) << "'>" << std::endl;
  openTag("arg_definitions", lvl + 2);
  node->arg_definitions()->accept(this, lvl + 4);
  closeTag("arg_definitions", lvl + 2);
  openTag("body", lvl + 2);
...
void til::xml_writer::do_declaration_node(til::declaration_node * const node, int lvl) {
  // ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') + "<" + node->label();
  os() << " qualifier='" << qualifier_to_str(node->qualifier()) << "'";
  os() << " type='" << type_to_str(node->type()) << "'";
  os() << " identifier='" << node->identifier() << "' />" << std::endl;

  if (node->expression() == nullptr) {
    os() << std::string(lvl + 2, ' ') + "<value />" << std::endl;
  } else {
    openTag("value", lvl + 2);
...
void til::xml_writer::do_call_node(til::call_node * const node, int lvl) {
  ...
  openTag(node, lvl);
  openTag("call", lvl + 2);
  if (node->function_ptr() == nullptr) {
    os() << std::string(lvl + 4, ' ') + "<recursive />" << std::endl;
  } else {
    node->function_ptr()->accept(this, lvl + 4);
  }
  closeTag("call", lvl + 2);
  openTag("args", lvl + 2);
